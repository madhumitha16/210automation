<?php
  ...
  //Admin details Update
     if(isset($_POST['username'])){
		...
		$sqlup = mysqli_query($conn, "update user set uname = '".$name."', email = '".$email."', lastname = '".$lname."' where id = '".$id."'");
		if($sqlup){
    ...
    if(isset($_POST['oldpassword'])){
		...
		$qry1 = mysqli_query($conn, "select password from user where id = '".$id."'");
		$fet1 = mysqli_fetch_array($qry1);
		$oldrealpwd = $fet1['password'];
		
		if($newpassword == $confpassword) {
		if( $oldrealpwd == $oldpassword ) {
		$sqlpass = mysqli_query($conn, "update user set password = '".$newpassword."' where id = '".$id."'");
		if($sqlpass){
                echo "<div class='alert alert-success alert-dismissible' role='alert'>
